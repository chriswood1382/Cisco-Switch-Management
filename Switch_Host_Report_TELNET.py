...
###########################################################################################################################
def show_version(telnet_conn):
   
    telnet_conn.write("show version\n")
    
    time.sleep(1)
    version = telnet_conn.read_very_eager()
...
def show_mac(telnet_conn, show, switch_model, host_name):
    ...
    if (switch_model == 'WS-C2960' or 
        ...
        #run show command on switch and assign string output to cluster_member 	
        
        telnet_conn.write("show cluster member\n")
    
        time.sleep(1)
        cluster_member = telnet_conn.read_very_eager()
        ...
        for member in members:
            ...
            num, mac, name, interface, var, var2, interface2, state = member.split()
    ...
    for key in sorted(member_dict.keys()):
        ...
        if key == 0:
            ...
            time.sleep(1)
            telnet_conn.write(show + '\n')
    
            time.sleep(1)
            mac_table = telnet_conn.read_very_eager().split('\n')
            
            write_config(telnet_conn, member_dict[key][0])
            ...
            switch_model = show_version(telnet_conn)
...
########################################################################################################################### 
def calc_jack (telnet_conn, mac_dict, host_name):

    jack_dict = {}   

    #run show command to report all description lines
    time.sleep(1)
    telnet_conn.write("show conf | inc description\n")
    
   
    time.sleep(1)
    description = telnet_conn.read_very_eager()
...
###########################################################################################################################
def find_router(telnet_conn):
    
    #pull default gateway information from config, set this as the router for the switch, and then return router IP
    time.sleep(1)
    telnet_conn.write("show conf | inc ip default-gateway\n")
    
    time.sleep(1)
    show_conf = telnet_conn.read_very_eager().split('\n')
    
    show_conf = show_conf[1:-1]
    
    temp1, temp2, router = show_conf[0].split()
...
def write_config(telnet_conn, switch_name):
    ...
    time.sleep(1)
    telnet_conn.write("show configuration\n")
    
    time.sleep(1)
    config = telnet_conn.read_very_eager().split('\n')
    ...
    ****************************************
    ...
    while host_name:
        ...
        try:
            ...
            #Connect via Telnet
            telnet_conn = telnetlib.Telnet(host_name, TELNET_PORT, TELNET_TIMEOUT)

            output = telnet_conn.read_until("sername:", READ_TIMEOUT)
    
            telnet_conn.write(user_name_sw + "\n")
    
            output = telnet_conn.read_until("sswrd:", READ_TIMEOUT)
   
            telnet_conn.write(my_pass_sw + "\n")
    
            time.sleep(1)
            telnet_conn.write("terminal length 0\n")
            ...
            mac_dict = show_mac(telnet_conn, show, switch_model, host_name)
            
            jack_dict = calc_jack(telnet_conn, mac_dict, host_name)
        
            router = find_router(telnet_conn)
